from generateAtmosphere import *
...
g = sns.heatmap(random_atmosphere, cmap='jet', norm = LogNorm(), cbar_kws={'label': r'$P/T$ [Pa/K]'})
...
print('Lidar:')
cloud_backscatter_lidar = MieBackscatter(num_time_bins, num_height_bins, random_atmosphere, refractive_lidar, lidar_wavelength, atmos_type, particle_radii)
...
print('Radar (Mie):')
cloud_backscatter_radar_mie = MieBackscatter(num_time_bins, num_height_bins, random_atmosphere, refractive_radar, radar_wavelength, atmos_type, particle_radii)
...
cloud_backscatter_radar_rayleigh = RadBackscatter(num_time_bins, num_height_bins, random_atmosphere, refractive_radar, radar_wavelength, atmos_type, particle_radii)
...
avg_eff_radius_mie = np.zeros(num_time_bins)
avg_eff_radius_rayleigh = np.zeros(num_time_bins)
for sec in range(num_time_bins):
	...
	sum_radius = 0
	sum_eff_radius_mie = 0
	sum_eff_radius_rayleigh = 0
	num = 0
	for al in range(num_height_bins):
		if random_atmosphere[al,sec] >= 200:
			...
			sum_eff_radius_mie += colour_ratio_mie[al,sec]
			sum_eff_radius_rayleigh += colour_ratio_rayleigh[al,sec]**(1/4)
			num += 1
	avg_colour_mie[sec] = sum_colour_mie/num
	avg_colour_rayleigh[sec] = sum_colour_rayleigh/num
	avg_radius[sec] = sum_radius/num
	avg_eff_radius_mie[sec] = sum_eff_radius_mie/num
	avg_eff_radius_rayleigh[sec] = sum_eff_radius_rayleigh/num
...
df['avg_eff_radius_mie'] = avg_eff_radius_mie * 1e6
df['avg_colour_rayleigh'] = avg_colour_rayleigh
df['avg_eff_radius_rayleigh'] = avg_eff_radius_rayleigh * 1e6
...
g = sns.regplot(x='avg_radius', y='avg_eff_radius_mie', color='#4F2683', scatter_kws={'alpha':0.3, 'edgecolor':'k'}, data=df)
g.set_xlabel(r'Average Radius [$\mathrm{\mu}$m]')
g.set_ylabel(r'Average Effective Radius (Mie) [$\mathrm{\mu}$m]')
plt.subplots_adjust(left=0.2, bottom=0.2, right=0.9, top=0.9)
plt.show()

plt.figure(11, figsize=(8, 6))
g = sns.regplot(x='avg_radius', y='avg_eff_radius_rayleigh', color='#4F2683', scatter_kws={'alpha':0.3, 'edgecolor':'k'}, data=df)
g.set_xlabel(r'Average Radius [$\mathrm{\mu}$m]')
g.set_ylabel(r'Average Effective Radius (Rayleigh) [$\mathrm{\mu}$m]')
plt.subplots_adjust(left=0.2, bottom=0.2, right=0.9, top=0.9)
plt.show()
