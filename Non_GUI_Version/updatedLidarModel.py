    ...
    for sec in range(time):
        ...
        for al in range(height):
            if inpu[al,sec] >= 200:
                ...
                radii[al,sec] = particle_radii[al,sec]**2
                ...
                back[al,sec] = inpu[al,sec]/K * bigboy * particle_radii[al,sec]**6 * (2*np.pi/lambd)**(4) 
                radii[al,sec] = particle_radii[al,sec]**6    
    print()
    return back, radii

def rayleighBackscatter(time, height, inpu, n, wave, typ, particle_radii):
    back = np.zeros((height, time))
    radii = np.zeros((height, time))
    K = 1.38e-23
    bigboy = np.abs(((n**2 + 2)/(n**2 - 1)))**2
    for sec in range(time):
        ...
        for al in range(height):
            if inpu[al,sec] >= 200:
                back[al,sec] = inpu[al,sec]/K * bigboy * particle_radii[al,sec]**6 * (2*np.pi/wave)**(4)
                radii[al,sec] = particle_radii[al,sec]**6
            else:
                if wave == 532e-9:
                    m = 1.000278
                ...
                back[al,sec] = (inpu[al,sec]/K) * bigboy_2 * particle_radii[al,sec]**6 * (2*np.pi/wave)**(4)	
                radii[al,sec] = particle_radii[al,sec]**6
    print()
    return back, radii